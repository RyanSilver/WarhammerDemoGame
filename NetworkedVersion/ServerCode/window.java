...
public class window extends Application {
    ...
    Button egdUnit = new Button("Eldar Guardian Defenders");
       ...
       // scene1 = new Scene(pane1, 1200, 625);
        ...
        primaryStage.setTitle("warhammer game Server");
       ...
       phaseAdv.setLayoutY(600);
        ...
        phaseAdv.setOnAction(e->{
              if(turnM.getNetworkedTurn()==true)
            {
            turnM.incPhase();
...
/*
    takes the message from sockets, and changes the game state
    */
    
    public void decodeMessage(String m)
    {
         //example input
        //m = "50.0*100.0*1053.0*110.0*true*true50.0*200.0*1031.0*215.0*true*true50.0*300.0*1019.0*335.0*true*true50.0*400.0*1036.0*437.0*true*true";     
        String container1[] = m.split("F");
        int index =0;
        double redX;
        double redY;
        double blueX;
       ...
       for(int i =0;i<5;i++)
       {
           ...
           redA =  Boolean.parseBoolean(container1[index+4]);
           redTeam[i].alive = redA;
        
          
           if(redTeam[i].alive == false)
           {
               redTeam[i].die(root);
           }
           ...
           {
           redX = Double.parseDouble(container1[index]);
           redY= Double.parseDouble(container1[index+1]);
           redTeam[i].setPos(redX, redY);
           }
           
         
           blueA = Boolean.parseBoolean(container1[index+5]);
           blueTeam[i].alive = blueA;
           
           if(blueTeam[i].alive == false)
               blueTeam[i].die(root);
           else
           {
           blueX= Double.parseDouble(container1[index+2]);
           blueY= Double.parseDouble(container1[index+3]);
           blueTeam[i].setPos(blueX, blueY);
           }

           index+=6;
       }
    //
   //Example String: F151.0F122.0F1150.0F100.0FtrueFtrueF50.0F200.0F1150.0F200.0FtrueFtrueF50.0F300.0F1150.0F300.0FtrueFtrueF50.0F400.0F1150.0F400.0FtrueFtrue
//151-0, 122.0-1,115-2,100-3,true-4,true-5,50.0-6,200.0-7,1150-8,200-9,true-10,true-11     
        //elements of the array correspond to these values
        // red x pos - 0,6,12,18
        //red y pos - 1,7,13,19,24
        //blue x pos - 2,8,14,20,25
        //blue y pos - 3,9,15,21,26
        //red alive -4,10,16,22,27
        //blue alive - 5,11,17,23,28
    }
    
    /*
    Updates the game by getting information from client socket
    */
 public void updateGame() throws Exception
 {
     //data to be received 
     String message ="";
     //alive status/position status
     

     for(int i =0;i<5;i++)
     {
        
         message+=Double.toString(redTeam[i].getPosX());
         message+="F";
         message+=Double.toString(redTeam[i].getPosY());
         message+="F";
         message+=Double.toString(blueTeam[i].getPosX());
         message+="F";
         message+=Double.toString(blueTeam[i].getPosY());
         message+="F";
         message+=Boolean.toString(redTeam[i].alive);
         message+="F";
         message+=Boolean.toString(blueTeam[i].alive);
         message+="F";
        
     }
      
             turnM.changeNetworkedTurn();
             turnM.pass(blueTeam, redTeam);
             Platform.runLater(new Runnable() {
      @Override
      public void run() {
        setStatusUpdate("Opponent Turn");
      }
  });
